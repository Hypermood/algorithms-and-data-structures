...
class Solution {
    ...
    public static List<Vertex> shortestPath(Graph g, Vertex a, Vertex b){
        ...
        Map<Vertex,Vertex> cloud = new HashMap<>();
        AdaptablePQ pq = new AdaptablePQ();

        g.getAllVertices().stream().forEach(v->{
            if(v == a){
                dist.put(a,0);
            }
            else{
            ...
            pq.insertOrReplace(v,dist.get(v));

        });

        g.getAllVertices().stream().forEach(v->cloud.put(v,null));


        while(!pq.isEmpty()){

            VertexNumPair vertexNumPair = pq.removeMin();
            Vertex u = vertexNumPair.getVertex();
            int key = vertexNumPair.getNum();
            List<VertexNumPair> neighbours = g.getNeighbours(u);
...
//                }
                ...
                //if(cloud.get(v.getVertex()) == null){
                if(v.getNum() + dist.get(u) < dist.get(v.getVertex())){
                    dist.put(v.getVertex(), v.getNum() + dist.get(u));

                    cloud.put(v.getVertex(),u);
                    pq.insertOrReplace(v.getVertex(),dist.get(v.getVertex()));
                }
                //}
        ...
        return path.stream().filter(Objects::nonNull).collect(Collectors.toList());
